...
public class AroundAdvice {
	...
	//ProceedingJoinPoint : 지금 실행 중인 포인트컷 메소드, JoinPoint 인터페이스를 상속받아서 구현한 객체
	//						JoinPoint를 상속받아서 추가로 proceed라는 메소드를 구현한 객체
	//						proceed() : 포인트컷 메소드를 실행하는 메소드
	/*JoinPoint의 유용한 메소드들 
	 * 		getSignature() : 호출된 비즈니스 메소드의 시그니처(리턴타입, 이름, 매개변수)를 Signature객체로 담아서 리턴
	 ...
	 * aop:before, after-returning, after-throwing, after = JoinPoint 인터페이스를 직접 사용
	 ...
	 * 		toShortString() : 호출된 포인트컷 메소드의  리턴타입, 이름, 매개변수를 축약해서 String 타입으로 리턴
	 * 
	 */
	@Around("allPointcut()")
	public Object aroundAdvice(ProceedingJoinPoint pjp) throws Throwable {
		//ProceedingJoinPoint : 지금 실행 중인 포인트컷 메소드
