...
public class BoardController {
	...
	public String insertBoard(BoardVO vo) throws IOException {
		...
		//파일 업로드 처리
		MultipartFile uploaFile = vo.getUploadFile();
		if (!uploaFile.isEmpty()) {
			String fileName = uploaFile.getOriginalFilename();
//			uploaFile.transferTo(new File("D:/" + fileName));
			uploaFile.transferTo(new File("C:/Dev211/" + fileName));
		}
	...
	public String updateBoard(@ModelAttribute("board") BoardVO vo) {
		...
		//주소값이동 없이 화면만 바뀌면 되니깐 redirect를 안붙였다 책에서는..
		return "redirect:getBoardList.do";
	}
